...
node[:koality][:verification][:server_count].to_i.times do |server_num|
	...
	bash "Start cloud verification server #{server_num}}" do
		...
		code <<-EOH
			mkdir -p #{server_path}
			cd #{server_path}
			/home/#{node[:koality][:user]}/virtualenvs/2.7/bin/python #{node[:koality][:source_path][:internal]}/ci/platform/bin/start_verification_server.py -v #{server_path} -c >> #{server_path}/server.log 2>&1 &
			EOH
	end
end
