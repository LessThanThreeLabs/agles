...
def main():
	...
	instance_name = 'koality_snapshot_%d_%d' % cache_version

	print 'Creating new instance named "%s" based on image "%s"' % (instance_name, newest_global_image.name)
	virtual_machine = vm_class.from_directory_or_construct("/tmp/koality/snapshot/%d_%d" % cache_version, instance_name, newest_global_image.id)
	...
	virtual_machine.ssh_call('mv source /repositories/cached/%s' % primary_repository['name'])
