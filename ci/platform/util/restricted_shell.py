...
GIT_REPO_PATH_PATTERN = r"[^ \t\n\r\f\v']*\.git"
HG_REPO_PATH_PATTERN = r"[^ \t\n\r\f\v']*\.git"
...
class RestrictedHgShell(RestrictedShell):
	def __init__(self, valid_commands, user_id_commands):
		...
		self._hg_command_handlers = {
			'serve'
		}

		self._hg_command_args = {
		}
	...
	def _validate(self, repo_path):
		if not repo_path.endswith(".hg"):
			raise MalformedCommandError('repo_path: %s. Repositories must end in ".hg".' % repo_path)
		if ".." in repo_path:
			raise MalformedCommandError('repo_path: %s. Repository path cannot contain "..".' % repo_path)
