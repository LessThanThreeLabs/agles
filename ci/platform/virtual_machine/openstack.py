...
class OpenstackVm(VirtualMachine):
	...
	def provision(self, private_key, output_handler=None):
		return self.ssh_call("virtualenvs/2.7/bin/python -c \"from verification.box.provisioner import Provisioner; Provisioner('''%s''').provision()\"" % private_key, output_handler)
