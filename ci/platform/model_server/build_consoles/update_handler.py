	...
	def append_console_lines(self, build_id, read_lines, type, subtype):
		...
		with ConnectionFactory.get_sql_connection() as sqlconn:
			...
			existing_lines_query = console_output.select().where(
				or_(*(build_console.c.line_number == line_number for line_number in read_lines.iterkeys()))
			)
			existing_lines = [row[console_output.c.id] for row in sqlconn.execute(existing_lines_query)]
			sqlconn.execute(
				console_output.update().where(
					console_output.c.id == bindparam('id')
				).values(line=bindparam('line')),
				[{'id': line_id, 'line': read_lines[line_id]} for line_id in existing_lines]
			)
			sqlconn.execute(
