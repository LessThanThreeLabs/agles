...
class ReposUpdateHandler(ModelServerRpcHandler):
	...
	@AdminApi
	def set_github_hook(self, user_id, repo_id, hook_id, hook_secret, hook_type):
		github_repo_metadata = database.schema.github_repo_metadata

		update = github_repo_metadata.update().where(github_repo_metadata.c.repo_id == repo_id).values(hook_id=hook_id, hook_secret=hook_secret, hook_type=hook_type)
		with ConnectionFactory.get_sql_connection() as sqlconn:
			sqlconn.execute(update)
		self.publish_event('repos', repo_id, 'github hook added', hook_id=hook_id, hook_secret=hook_secret, hook_type=hook_type)
