...
class ChangesUpdateHandler(ModelServerRpcHandler):
	...
	def mark_change_finished(self, change_id, verification_status):
		self._update_change_status(change_id, verification_status,
			"change finished", end_time=int(time.time()))
		if verification_status == BuildStatus.FAILED:
			self._notify_failure(change_id)

	def mark_change_finished_with_merge_status(self, change_id, verification_status, merge_status):
		self._update_change_status(change_id, verification_status,
	...
	def _notify_failure(self, change_id):
		...
		if email is None:
			email = row[user.c.email] if row[user.c.id] > MAX_SPECIAL_USER_ID else row[commit.c.committer_email]
		if not email:
			return
