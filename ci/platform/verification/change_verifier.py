...
class ChangeVerifier(EventSubscriber):
	...
	def verify_change(self, verification_config, change_id, repo_type, workers_spawned, verify_only, patch_id=None):
		...
		def handle_other_result(task_result, change_failed, other_result_passed):
			if other_result_passed:
				return change_failed, other_result_passed
			if self._is_result_failed(task_result.result):
				if not change_failed:
					fail_change()
					return True, other_result_passed
			else:
				return change_failed, True

		def handle_default_result(task_result, change_failed):
		...
		while task_queue.has_more_results():
			...
			if task_result.type == 'other':
				change_failed, other_result_passed = handle_other_result(task_result, change_failed, other_result_passed)
			else:
				change_failed = handle_default_result(task_result, change_failed)
